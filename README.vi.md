...
**Công cụ thu thập dữ liệu Training Needs Analysis (TNA) cho Instructional Designers**, xây dựng theo phương pháp Action Mapping (Cathy Moore — *Map It*). Một file HTML duy nhất, chạy offline, không cần cài đặt.
...
Cách truyền thống là sử dụng hàng chục file rời — điền tay, nhập lại, tổng hợp thủ công.  
Discovery Hub gom tất cả dữ liệu về một chỗ theo nguyên tắc **nhập một lần, dữ liệu cập nhật vào mọi tài liệu đầu ra**.
...
- **Xuất TNA Package:** một file `.md` duy nhất chứa toàn bộ dữ liệu + hướng dẫn sẵn cho AI & đưa vào Claude là tạo được Action Map hoàn chỉnh
...
2. Làm **Bước 0** — nếu NO-GO, dừng lại và đọc khuyến nghị 
3. Đi lần lượt Bước 1 → 5, nhập đến đâu app tự lưu đến đó
4. **Export JSON** để backup 
...
| **2 · TNA Package** | Bấm *Xuất TNA Package* | `TNA_Package_[dự án].md` — mọi câu trả lời, ghi chú, transcript + hướng dẫn phân tích |
| **3 · Action Map** | Đưa file vào Claude và nói *"Tạo Action Map theo hướng dẫn trong file"* | Flowchart HTML: business goal → phân tích training giải quyết được không → behaviors → practice activities → danh sách bàn giao |
...
*Nếu công cụ này hữu ích với bạn, hãy thêm một lượt ⭐ star repo hoặc một lời giới thiệu đến đồng nghiệp L&D nhé.*
